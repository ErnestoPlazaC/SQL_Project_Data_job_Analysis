...
FROM 
    top_paying_remote_jobs
INNER JOIN skills_job_dim AS skills_to_job ON top_paying_remote_jobs.job_id = skills_to_job.job_id
INNER JOIN skills_dim AS skills ON skills_to_job.skill_id = skills.skill_id
ORDER BY
 salary_year_avg DESC

/*
Here are some key insights from the Skill Frequency by Type analysis:

Most Mentioned Skills:
    -SQL (8 times)
    -Python (7 times)
    -Tableau (6 times)
    -R, Snowflake, Pandas, Excel (3–4 times each)

Skill Type Breakdown:
    -Programming: 22 mentions (highest)
    -Analyst Tools: 13 mentions
    -Cloud: 10 mentions
    -Libraries: 8 mentions
    -Async and Other: 4–9 mentions
*/